...
private:
    ...
    vector<vector<int> > MaptoGrid()
    {
        ...
        vector<vector<double> > mymap = GetMap();
...
void visualization(Map map, Planner planner)
{
    ...
    plt::plot({planner.start[0]},{planner.start[1]}, "bo");
    plt::plot({planner.goal[0]},{planner.goal[1]}, "b*");
    // TODO: Plot the robot path in blue color using a .
    for (double x = 0; x < planner.path.size(); x++) {
            plt::plot({planner.path[x][0]},{planner.path[x][1]}, "b.");
    }
